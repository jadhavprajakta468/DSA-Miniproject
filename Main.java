...
class Ticket {                    //class for ticket info 
	...
	Ticket(String customerName, String movieName, int prize, int seatnum, String slot, String seatType,int tp, float gst, float totalp) {
...
class MovieTicketBookingSystem {                            //class for booking system 
	...
	void about_movie(String movieName) {                      //method for info of movies 
		...
		if (movieName.equalsIgnoreCase("Gadar2")) {
			System.out.println("Gadar 2: A Nation Divided is a compelling drama set in a divided nation.\nFollow the passionate love story of Aryan and Zara, whose relationship challenges societal norms amid political unrest.\nTheir love becomes a beacon of hope and unity in a turbulent world.\n");
		}
		else if (movieName.equalsIgnoreCase("jawan")) {
			System.out.println("In this gripping drama, 'Jawan' follows the intense journey of a young man named Jai, who finds himself torn between his duty towards his country and responsibilities towards his family.\n Amidst high-stakes situations, Jai must make difficult choices to protect both his loved ones and his nation.\n 'Jawaan' is a thrilling tale of sacrifice, patriotism, and moral dilemmas.");
		}
		else if (movieName.equalsIgnoreCase("KGF:Chapter2")) {
			System.out.println("Rocky successfully rises as the leader and saviour of the people of the kalor Gold fields.\nBut ,in his goal tonfulfil his mother's wishes, Rocky must tackle Adheera,Inayat Khalil and Ramika Sen.");
		}
	}
	...
	void Recliner(String movieName, String slot, int available, int seat) {                //method for recliner seat allotment 
		...
		if (available > 0) {
			...
			int p1 = 400;
			addNode(customerName,movieName,prize1,seat, slot,seatType,p1);
		}
		else {
	...
	void addNode(String customerName, String movieName, int prize, int seatno, String slot, String seatType,int originalPrize) {                      //creating and adding LL of single each tickets 
		System.out.println(" ");
		...
		System.out.println();
		Ticket new_node = new Ticket(customerName, movieName, prize, seatno, slot, seatType,tp,gst,totalp);
	...
	void DisplayTicket() {                                    //displaying the ticket LL  for display function in main
		...
		while (curNode != null) {
			...
			displayBox("Details of your ticket");
	        displayInfo("Name", curNode.customerName);
	        displayInfo("Movie", curNode.movieName);
	        displayInfo("Seat type", curNode.seatType);
	        displayInfo1("Seat number", curNode.seatnum);
	        displayInfo("Slot", curNode.slot);
	        displayBox(""); // Close the box			
	        curNode = curNode.next;

		}

	}
	static void displayBox(String title) {
        int boxWidth = 40; // Adjust as needed
        System.out.println("+" + "-".repeat(boxWidth - 2) + "+");
        if (!title.isEmpty()) {
            System.out.printf("|%-" + (boxWidth - 3) + "s |\n", title);
            System.out.println("+" + "-".repeat(boxWidth - 2) + "+");
        }
    }

    // Method to display information within the box
    static void displayInfo(String label, String slot) {
        int labelWidth = 12; // Adjust as needed
        System.out.printf("| %-" + (labelWidth - 1) + "s: %-" + (40 - labelWidth - 4) + "s|\n", label, slot);
    }
    static void displayInfo1(String label, float gst) {
        int labelWidth = 12; // Adjust as needed
        System.out.printf("| %-" + (labelWidth - 1) + "s: %-" + (40 - labelWidth - 4) + "s|\n", label, gst);
    }
	void DisplayBill() {                                        //billiing option 
		...
		while (curNode != null) {
			...
			displayInfo1("Seat number", curNode.seatnum);
			displayInfo("Movie", curNode.movieName);
			displayInfo1("T.price", curNode.tp);
			displayInfo1("GST(28%GST)", curNode.gst);
			if (ans == 1) {
				displayBox("Popcorn charges:100/-");
			}
			displayInfo1("Total price", curNode.totalp);
			displayBox(""); // Close the box	
			curNode = curNode.next;
		}
	}
	void delete() {                                                                  //Deleting ticket 
		...
		if (s == 1) {
			...
			if (temp == null) {                                                //no tickets booked
				System.out.println("There is no Ticket booking present.");
				return;
			}

			else if (temp != null && temp.seatnum == seatNum) {                        //seat at start head (booked recently)
				...
				while (temp != null && temp.seatnum != seatNum ) {                //seat in middle (booked moderately)
...
About Movie:
Gadar 2: A Nation Divided is a compelling drama set in a divided nation.
Follow the passionate love story of Aryan and Zara, whose relationship challenges societal norms amid political unrest.
Their love becomes a beacon of hope and unity in a turbulent world.
